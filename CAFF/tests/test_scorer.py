...
Verify scorer (Eqs. 18-19) properties:
  ...
  • s ∈ (0, 1)                                        (Eq. 19, sigmoid)
  • W^ctx computed once per hop, reused per candidate (S3)
"""
...
def test_depth_correction_shape():
    """A_ℓ B_ℓ^T has shape (d, d) regardless of ρ."""
...
def test_hop_scorer_W_ctx_decomposition():
    """Eq. 18: W^ctx_ℓ = W_0 + A_ℓ B_ℓ^T + Δ_ℓ"""
    d, rho = 32, 4
...
def test_dbm_can_be_disabled():
    ...
    # Without DBM, W^ctx_ℓ = W_0 + A_ℓ B_ℓ^T (constant across batch)
