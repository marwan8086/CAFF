...
Verify evaluation primitives:
  ...
  â€¢ Precision/Recall/F1 match reference values
  â€¢ MAP, NDCG@10 sane on toy inputs
  â€¢ JSD diagnostic returns 0.00 for context-agnostic baseline
  â€¢ Paired bootstrap returns sensible p-value
"""
...
def test_average_precision_perfect_ranking():
    """All positives ranked above all negatives â†’ AP = 1.0."""
...
def test_jsd_zero_when_equal():
    """JSD between identical distributions is 0 â€” the CBE signature
    (paper Table 10: every context-agnostic baseline â†’ 0.00 bits)."""
...
def test_jsd_paper_appendix_c_calculation():
    """Reproduce the JSD calculation in paper Appendix C verbatim:
        p=0.872, q=0.238 â†’ 2Â·JSD â‰ˆ 1.84 bits.
    """
    p, q = 0.99, 0.01
...
def test_paired_bootstrap_zero_difference():
    """Identical metrics â†’ delta â‰ˆ 0, p â‰ˆ 1."""
...
def test_paired_bootstrap_clear_difference():
    """Strongly different metrics â†’ small p-value."""
