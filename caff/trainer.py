...
class CheckpointManager:
    ...
    def save(
        ...
        payload = {
            ...
            "ablation": asdict(self._ablation) if hasattr(self, "_ablation") else None,
        ...
        if is_better:
            ...
            logger.info(
                f"  [OK] New best ({self.metric}={current:.4f}) saved to {best_path}"
            )
...
class CAFFTrainer:
    ...
    def __init__(
        ...
        # Fail-fast: empty datasets indicate upstream pipeline failure
        # (KG load, BFS extraction, or gold annotation produced nothing).
        if len(train_dataset) == 0:
            raise RuntimeError(
                f"Training dataset is empty (0 triple instances). "
                f"Check kg_path='{config.kg_path}', min_relation_freq, "
                f"BFS extraction at L={config.L}, and gold-annotation. "
                f"Refusing to start training to avoid silent zero-metric runs."
            )
        if len(dev_dataset) == 0:
            raise RuntimeError(
                f"Dev dataset is empty (0 triple instances). "
                f"Check dev_path='{config.dev_path}'. "
                f"Refusing to start training without a validation set."
            )
    ...
    def _compute_hc3_loss(self) -> torch.Tensor | None:
        ...
        for a, n in zip(anchors, negatives):
            # Use the actual natural-language question text (paper §6.4 requires
            # encoding the real query, not the query_id string).
            q_a = self._get_query_embedding(a.query_id, a.question)
            q_n = self._get_query_embedding(n.query_id, n.question)
    ...
    def _train_one_epoch(self, epoch: int) -> dict[str, float]:
        ...
        # Defensive check: per-epoch verification that work will actually happen.
        n_query_hop_groups = sum(
            1 for _ in self.train_dataset.iter_by_query_hop()
        )
        if n_query_hop_groups == 0:
            raise RuntimeError(
                f"Epoch {epoch}: iter_by_query_hop() yielded zero (query, hop) "
                f"groups despite train_dataset having {len(self.train_dataset)} "
                f"instances. This indicates a bug in CAFFTripleDataset.iter_by_query_hop."
            )
    ...
    def train(self) -> TrainingHistory:
        ...
        Note: global seeding is performed once at the start of train.py
        (the entry point). Re-seeding here would unnecessarily reset
        any RNG state that has advanced during model/encoder construction.
        """
        # Clear stale jsonl log from previous runs (append mode would
        # otherwise accumulate epochs across restarts).
        if self.log_jsonl_path is not None and self.log_jsonl_path.exists():
