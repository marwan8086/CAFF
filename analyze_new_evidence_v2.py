"""
analyze_new_evidence_v2.py - FIXED: Reads ALL target/*.parquet parts
(previous version read only part-00000).
"""
...
# ============================================================
# Step 1: Build Ensembl -> HGNC mapping from ALL target parts
# ============================================================
print("=" * 75)
print("  Step 1: Load target metadata (Ensembl -> HGNC mapping)")
print("=" * 75)

target_parts = sorted((ROOT / "target").glob("*.parquet"))
print(f"  Reading {len(target_parts)} target parts...")
target = pd.concat(
    [pq.read_table(p).to_pandas() for p in target_parts],
    ignore_index=True
)
print(f"  Loaded {len(target):,} target rows (TOTAL across all parts)")
...
# ============================================================
# Step 2: Build MONDO -> Orphanet mapping
# ============================================================
print()
print("=" * 75)
print("  Step 2: Load disease metadata (-> Orphanet mapping)")
print("=" * 75)
...
for source_name, source_dir in evidence_sources.items():
    ...
    if disease_col is None:
        print(f"  SKIP: no disease column")
        continue
    ...
    print(f"  Skipped no gene:    {skipped_no_gene:,}")
    print(f"  Skipped no disease: {skipped_no_disease:,}")
    print(f"  Skipped no orph:    {skipped_no_orph:,}")

# ============================================================
# Step 4: Combine + compare with KG v2
# ============================================================
print()
print("=" * 75)
...
print(f"  Total pairs (all sources, raw): {len(pairs_df):,}")
...
print(f"  OTG unique pairs:     {len(otg_pair_keys):,}")
print(f"  Already in KG v2:     {len(existing_keys):,} ({100*len(existing_keys)/max(len(otg_pair_keys),1):.1f}%)")
print(f"  NEW pairs:            {len(new_keys):,} ({100*len(new_keys)/max(len(otg_pair_keys),1):.1f}%)")
...
print(f"\n  New pairs by source:")
print(new_pairs['source'].value_counts().head(15).to_string())

print(f"\n  Sample new pairs (first 10):")
for _, r in new_pairs.head(10).iterrows():
    print(f"    Orphanet:{r['orphanet_id']:>8} -- {r['gene_symbol']:<12} ({r['source']})")
...
print(f"Summary:")
print(f"  KG v2 existing pairs:    {len(kg_pairs):,}")
print(f"  OTG extracted unique:    {len(otg_pair_keys):,}")
print(f"  NEW for KG v3:           {len(new_keys):,}")
print(f"  Expected edge growth:    +{len(new_keys)*2:,} (bidirectional)")
